 ...
 * USART driver task.
 *
 * This implements two parallel state machines, the transmitter and the
 * receiver.  They are mostly independent, but share an interrupt, so they must
 * be entangled.
 *
 * There are four ports associated with this driver:
 * - P_TX allows interaction with the transmitter.  It is unmasked only when
 *   transmission queue slots are available.
 * - P_RX allows interaction with the receiver.  It is unmasked only when 
 *   received data is available.
 * - P_IRQ receives interrupts from the kernel.  (Never masked.)
 * - P_MON receives heartbeat requests from the driver monitor.  (Never masked.)
 */
...
int task_main() {
  while (true) {
    ...
    switch (m.brand) {
      case P_IRQ: handle_irq(m); break;
      case P_TX:  handle_tx(m); break;
      case P_RX:  handle_rx(m); break;
      case P_MON: handle_mon(m); break;
    }
  }
}
